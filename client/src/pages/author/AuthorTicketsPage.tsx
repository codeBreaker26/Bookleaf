...
export function AuthorTicketsPage() {
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6">
      <section className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-6 shadow-2xl shadow-slate-950/20 backdrop-blur-xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-slate-500">My Tickets</p>
            <h1 className="mt-2 text-3xl font-semibold text-white">Author tickets</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-400">
              Track the status of your support requests and follow up directly on the issue thread.
            </p>
          </div>
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-900/95 shadow-2xl shadow-slate-950/20">
        <div className="border-b border-slate-800 px-6 py-5 bg-slate-950/90">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">Select any ticket to check the latest updates.</p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8">
            <EmptyState title="No tickets found" description="Create a new ticket or adjust your search to find support requests." />
          </div>
        ) : (
            ...
            <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
              ...
              <tbody>
                {filteredTickets.map((ticket) => (
                  <TicketTableRow key={ticket.id} ticket={ticket} onClick={() => navigate(`/author/tickets/${ticket.id}`)} />
                ))}
              </tbody>
            </table>
          </div>
